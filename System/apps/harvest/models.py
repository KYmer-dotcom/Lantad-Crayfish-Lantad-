...
class HarvestSchedule(models.Model):
    ...
    stock_batch = models.ForeignKey('stock.StockBatch', on_delete=models.CASCADE, related_name='harvest_schedules')
    ...
    def __str__(self):
        return f"{self.stock_batch.batch_code} - {self.scheduled_date}"
...
class HarvestRecord(models.Model):
    ...
    stock_batch = models.ForeignKey('stock.StockBatch', on_delete=models.CASCADE, related_name='harvest_records')
    ...
    def __str__(self):
        return f"{self.stock_batch.batch_code} - {self.harvest_date}"

    @property
    def total_graded_quantity(self):
        return (self.grade_a_quantity or 0) + (self.grade_b_quantity or 0) + (self.grade_c_quantity or 0)

    @property
    def quality_evaluation(self):
        total = self.total_graded_quantity
        if total <= 0:
            return 'Not Evaluated'
        grade_a_pct = (self.grade_a_quantity / total) * 100
        grade_b_pct = (self.grade_b_quantity / total) * 100
        if grade_a_pct >= 60:
            return 'Premium'
        if grade_a_pct + grade_b_pct >= 80:
            return 'Standard'
        return 'Mixed'
